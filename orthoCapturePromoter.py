...
#        with open(querycsv, "r") as f:
#                with open(query, "a") as o:
#                        s = pd.read_csv(f,encoding='utf-8')
#                        for i in range(len(s)):
#                                if not pd.isnull(s.iloc[i][1]):
#                                        if pd.isnull(s.iloc[i][2]):
#        #                                        print(type(s.iloc[i][1]))
#        #                                       print(type(s.iloc[i][3]))
#                                                o.write(">"+str(s.iloc[i][1])+"_"+str(s.iloc[i][3])+"\n")
#                                                o.write(s.iloc[i][4]+"\n")
#                                        else:
#                                                o.write(">"+str(s.iloc[i][1])+"_"+str(int(s.iloc[i][2]))+"_"+str(s.iloc[i][3])+"\n")
...
#                        print(start)
				...
				## keep everything because I can't expect promoters to have matches
#                        print(stop)
        ...
        with open(name+"/"+name+"_"+genename+"_match_all_tblastx.fa", "a") as outfile:
                ...
                for i in genevalues:
                        ...
                        for fname in genefiles:
                                if sum(1 for _ in fname) > 1:
                                        openfile = open(fname, "r")
                                        if openfile.readlines()[1].strip():
                                                print(fname)
                                                print(name)
...
#                                print("Already put a reference sequence in "+filename)
                                ...
                                try:
                                        seq = allquery[(allquery["Gene Name Shorthand"] == genename) & (allquery["Exon Number"].apply(str) == str(exon)) & (allquery["Reference Species"].str.contains(ref))]["Reference Sequence"].to_string().split()[1] # convert pandas entry to string, get rid of row identifier
#                                        print(seq)
                                        print(filename)
...
def align(name,filename,querycsv):
        ...
        allquery = pd.read_csv(querycsv,dtype=object)
        subject = allquery[(allquery["Gene Name Shorthand"] == geneshort) & (allquery["Exon Number"].apply(str) == str(exonnumber)) & (allquery["Reference Species"].str.contains(reference))]["Reference Sequence"].to_string().split()[1]
        with open(genename+"_reference.fasta","w") as ref:
                ref.write(">"+reference+"\n")
                ref.write(subject+"\n")
        if os.path.exists(name+"/"+name+"_"+genename+"_match_all_tblastx-merged.fasta") and os.stat(name+"/"+name+"_"+genename+"_match_all_tblastx-merged.fasta").st_size != 0:
                with open(name+"/"+name+"_"+genename+"_match_all_tblastx-merged.fasta", "r") as allfile:
                        ...
                        if max(length) < 120:
                                with open(name+"_tblastx.log","a") as log:
...
#                        print(contigs[0][0].split()[0]) # have to split contigs[0][0] to get rid of " Reversed:" if exists
                        ...
                        if samecontigs: # if all contigs the same
                                ...
                                tblastx_command_line = 'ncbi-blast-2.2.29+/bin/blastn -query '+genename+"_reference.fasta"+' -subject '+name+"/"+name+"_"+genename+"_temp.fa"+' -out '+name+"/"+name+'_'+genename+"_"+start[best]+"-"+stop[best]+"_"+contigs[0][0].split()[0]+'_align.out -outfmt "6 sseqid qstart qend sstart send qcovhsp slen qlen bitscore evalue" -max_target_seqs 1'
                ...
                print("Merged file does not exist or is empty, cannot align: "+filename)
...
def tblastx(name,querycsv,query,genome):
        ...
        tblastx_command_line = 'ncbi-blast-2.2.29+/bin/blastn -query '+query+' -db '+genome+' -culling_limit 1 -out '+name+'/'+name+'_tblastx.out -outfmt "6 qseqid qstart qend sstart send qcovhsp bitscore sacc" -max_target_seqs 1'
        ...
        #count = defaultdict(int) # number of lines in _tblastx.out corresponding to gene name
        fasta = defaultdict(list)
        for i in reader:
                fasta[i[0]].append([i[3],i[4],i[7]]) # gene into dictionary with start, stop, sequence, and contig as values
        #        count[gene] += 1
        #        with open(name+"_"+gene+"_match_"+str(count)+"_tblastx.fa","w") as n:
        #                n.write(">"+i[0]+"\n")
        #                n.write(i[7]+"\n")
        f.close()
...
def recBlast(fasta,gn,name,genome):
        for x in range(len(fasta[gn])): # for each instance of that gene
                ...
                with open(filename,"w") as n: # create a new file with the name and sequence
                        ...
                        n.write(sequence+"\n")
                print(filename)
#                blast_command_line = 'ncbi-blast-2.7.1+/bin/blastn -task dc-megablast -db ../../../../../../../N/dc2/scratch/marylaut/nt/nt -query '+filename+' -max_hsps 50 -out '+name+"/"+name+"_"+gn+"_"+fasta[gn][x][0]+'-'+fasta[gn][x][1]+'_blast.out -outfmt "6 qseqid qcovhsp stitle"'
#                subprocess.Popen(blast_command_line, shell=True).wait() # blast that sequence
		## don't need to reciprocal blast because not checking if there are promoter matches
...
#def recBlast(name,genome):
#        with open('fasta_tblastx.pickle', 'rb') as handle:
#                fasta = pickle.load(handle)
#        for gn in fasta.keys(): # for each gene in dictionary
#                for x in range(len(fasta[gn])): # for each instance of that gene
#                        filename = name+"_"+gn+"_"+fasta[gn][x][0]+'-'+fasta[gn][x][1]+"_tblastx.fa"
#                        with open(filename,"w") as n: # create a new file with the name and sequence
#                                start = min(fasta[gn][x][0], fasta[gn][x][1])
#                                stop = max(fasta[gn][x][0], fasta[gn][x][1]) # because blastdbcmd can't handle start greater than stop
#                                n.write(">"+gn+"_"+start+"-"+stop+"_"+fasta[gn][x][2]+"\n")
...
#                                n.write(sequence+"\n")
#                        print(filename)
#                        blast_command_line = 'blastn -task dc-megablast -db nt -query '+filename+' -remote -max_hsps 50 -out '+name+"_"+gn+"_"+fasta[gn][x][0]+'-'+fasta[gn][x][1]+'_blast.out -outfmt "6 qseqid qcovhsp stitle"'
#                        subprocess.Popen(blast_command_line, shell=True).wait() # blast that sequence
#        #            for tx in range(count[gn]):
#        #                print(name+'_'+gene+'_match_'+str(tx)+'_tblastx.fa')
#        #                blast_command_line = 'ncbi-blast-2.7.1+/bin/blastn -task dc-megablast -db nt -query '+name+"_"+gene+'_match_'+str(tx)+'_tblastx.fa -remote -max_hsps 50 -out '+name+"_"+gene+'_match_'+str(tx)+'_blast.out -outfmt "6 qseqid qcovhsp stitle"'
#        #                subprocess.Popen(blast_command_line, shell=True).wait()
#        with open('fasta_recblast.pickle', 'wb') as handle:
#                pickle.dump(fasta, handle, protocol=pickle.HIGHEST_PROTOCOL)
...
#                newi="_".join(i.split("_")[:-1])
                is_gene(name,i,j)
#        is_missing(name,querycsv) # don't care because promoters
        for i,j in fasta.items():
#                newi="_".join(i.split("_")[:-1])
...
#        if skip == "noskip":
#                csv_to_fasta(querycsv,query)
#                tblastx_command_line = 'ncbi-blast-2.2.29+/bin/tblastx -query '+query+' -db '+genome+' -culling_limit 1 -out '+name+'_tblastx.out -outfmt "6 qseqid qstart qend sstart send qcovhsp bitscore sacc" -max_target_seqs 1'
#                        # using ncbi blast version 2.2.29 so sseq outputs nucleotide sequence (2.6.* outputs protein sequence)
#                subprocess.Popen(tblastx_command_line, shell=True).wait() # shell because of argument in quotes, wait so script doesn't advance until done
#                        # will output file [name]_tblastx.out
#                f = open(name+"_tblastx.out","r")
#                reader = csv.reader(f, delimiter = "\t")
#        #        count = defaultdict(int) # number of lines in _tblastx.out corresponding to gene name
#                fasta = defaultdict(list)
#                for i in reader:
...
#                            n.write(sequence+"\n")
#                        print(filename)
#                        blast_command_line = 'ncbi-blast-2.7.1+/bin/blastn -task dc-megablast -db nt -query '+filename+' -remote -max_hsps 50 -out '+name+"_"+gn+"_"+fasta[gn][x][0]+'-'+fasta[gn][x][1]+'_blast.out -outfmt "6 qseqid qcovhsp stitle"'
#                        subprocess.Popen(blast_command_line, shell=True).wait() # blast that sequence
#        #            for tx in range(count[gn]):
#        #                print(name+'_'+gene+'_match_'+str(tx)+'_tblastx.fa')
#        #                blast_command_line = 'ncbi-blast-2.7.1+/bin/blastn -task dc-megablast -db nt -query '+name+"_"+gene+'_match_'+str(tx)+'_tblastx.fa -remote -max_hsps 50 -out '+name+"_"+gene+'_match_'+str(tx)+'_blast.out -outfmt "6 qseqid qcovhsp stitle"'
#        #                subprocess.Popen(blast_command_line, shell=True).wait()
#                with open('fasta.pickle', 'wb') as handle:
#                        pickle.dump(fasta, handle, protocol=pickle.HIGHEST_PROTOCOL)
#                for i,j in fasta.items():
#                    is_gene(name,i,j)
...
#        elif sys.argv[4] == "recblast":
#                with open('fasta.pickle', 'rb') as handle:
#                    fasta = pickle.load(handle)
#                for i,j in fasta.items():
#                    is_gene(name,i,j)
#                is_missing(name,querycsv)
...
#        elif sys.argv[4] == "blastmatch":
##                f = open(name+"_tblastx.out","r")
##                reader = csv.reader(f, delimiter = "\t")
##        #        count = defaultdict(int) # number of lines in _tblastx.out corresponding to gene name
##                fasta = defaultdict(list)
##                for i in reader:
##                    fasta[i[0]].append([i[3],i[4],i[7]])
##                f.close()
#                with open('fasta.pickle', 'rb') as handle:
#                    fasta = pickle.load(handle)
#                for i,j in fasta.items():
